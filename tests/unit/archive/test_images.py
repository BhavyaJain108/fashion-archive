...
@pytest.fixture()
def env(tmp_path):
    cat = Catalog(DirectoryObjectStore(tmp_path / "objects"))
...
def test_archive_stores_content_addressed_bytes_and_records_the_url(env):
    cat, url, store, sink, _ = env
    counter: dict = {}
    saved = store.archive(
        make_transport(counter), cat, url, "kuurth.com", ["https://cdn.shopify.com/nemo-1.jpg"]
    )
    assert saved == 1
...
def test_known_urls_are_never_redownloaded(env):
    ...
    cat, url, store, _, _ = env
    counter: dict = {}
    t = make_transport(counter)
    store.archive(t, cat, url, "kuurth.com", ["https://cdn.shopify.com/nemo-1.jpg"])
    saved = store.archive(t, cat, url, "kuurth.com", ["https://cdn.shopify.com/nemo-1.jpg"])
    assert saved == 0
...
def test_width_param_appended_for_shopify_cdn(env):
    ...
    cat, url, _, sink, _ = env
    store = ImageStore(sink, width=1200)
    counter: dict = {}
    store.archive(
        make_transport(counter), cat, url, "kuurth.com", ["https://cdn.shopify.com/nemo-1.jpg?v=3"]
    )
    ...
    assert cat.known_image_urls("kuurth.com", url) == {"https://cdn.shopify.com/nemo-1.jpg?v=3"}
...
def test_non_shopify_urls_are_fetched_unmodified(env):
    ...
    counter: dict = {}
    store.archive(
        make_transport(counter), cat, url, "kuurth.com", ["https://img.example.com/a.jpg"]
    )
...
def test_bad_response_is_skipped_not_fatal(env):
    ...
    assert store.archive(t, cat, url, "kuurth.com", ["https://cdn.shopify.com/gone.jpg"]) == 0
