...
class IntelligentScraper:
    ...
    def _scrape_page_intelligently(self, url: str) -> Tuple[List[Product], List[str]]:
        ...
        try:
            ...
            if len(response.content) == 0:
                print("❌ Empty response - skipping this URL")
            ...
            if self._is_js_rendered_page(soup):
                print("⚠️  Page appears to be JavaScript-rendered - attempting HTML extraction anyway")
            ...
            if decision.action == 'scrape_products':
                ...
                # If no products found, report it
                if len(products) == 0:
                    print("❌ No products found with HTML extraction")
                    errors.append("No products found on page")
    ...
    def _scrape_collection_progressively(self, collection_url: str, collection_name: str, max_products: int) -> Tuple[List[Product], List[str]]:
        ...
        try:
            ...
            # Report final results with HTML-only extraction
            if len(actual_products) < max_products:
                print(f"⚠️  HTML extraction found {len(actual_products)} products for {collection_name} (less than target {max_products})")
                print("   (Browser automation disabled - HTML-only mode)")
                if len(actual_products) == 0:
                    errors.append(f"No products found with HTML extraction for {collection_name}")
    ...
    # Browser automation removed - HTML-only mode
