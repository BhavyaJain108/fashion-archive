...
const TERMS = [
  ...
  ['Buttons', 'run now brings the brand’s next turn to now; a worker takes it within ten seconds. On a paused brand it reads run once: one run, then still paused. pause takes effect at the next claim, so a brand being scraped reads pause after run and finishes first; resume puts it back on its cadence, and an overdue brand runs at once. release appears only when the worker holding a brand has not beaten for twelve minutes; it hands the brand back and the run starts again from nothing. Nothing here is refused silently — a refusal is written under the row.'],
