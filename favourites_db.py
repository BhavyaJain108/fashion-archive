...
class FavouritesDB:
    ...
    def _copy_image_to_favourites(self, original_path, season_data, collection_data, look_data):
        """Copy image to favourites directory and return new path"""
        try:
            original_file = Path(original_path)
            if not original_file.exists():
                print(f"Warning: Original image not found: {original_path}")
                return original_path  # Return original path if file doesn't exist
            ...
            if not favourite_path.exists():
                shutil.copy2(original_file, favourite_path)
                print(f"Copied image to favourites: {favourite_path}")
            else:
                print(f"Image already exists in favourites: {favourite_path}")
            
            return str(favourite_path)
            
        except Exception as e:
            print(f"Error copying image to favourites: {e}")
            return original_path  # Return original path on error
    
    def _remove_image_from_favourites(self, favourite_path):
        """Remove image from favourites directory"""
        try:
            path = Path(favourite_path)
            if path.exists() and path.parent == self.favourites_dir:
                path.unlink()
                print(f"Removed image from favourites: {favourite_path}")
                return True
        except Exception as e:
            print(f"Error removing image from favourites: {e}")
        return False
