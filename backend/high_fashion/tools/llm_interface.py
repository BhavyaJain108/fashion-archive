                    ...
                    if hasattr(content_block, "type") and content_block.type == "tool_use":
                        if hasattr(content_block, "input"):
                            ...
                            if debug:
                                ...
                                print(
                                    f"   Keys: {list(tool_input.keys()) if isinstance(tool_input, dict) else 'not a dict'}"
                                )
                                ...
                                if tool_input:
                                    ...
                                    print(
                                        f"   Content preview: {json.dumps(tool_input, indent=2)[:500]}"
                                    )
    ...
    def generate(
        ...
        if response_model:
            ...
            if not choice.message.tool_calls:
                raise ValueError(
                    f"No tool_call in OpenRouter response. content={choice.message.content!r}"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"OpenRouter tool args were not valid JSON: {e}; raw={tool_args!r}"
                )
        else:
            response = self.client.chat.completions.create(
                model=self.model,
