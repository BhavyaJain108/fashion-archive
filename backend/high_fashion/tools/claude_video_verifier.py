...
class ClaudeVideoVerifier:
    ...
    def verify_video_matches(
        self, search_query: str, videos: List[VideoResult]
    ) -> VerificationResult:
        """
    ...
    def _download_video(self, video: VideoResult) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"📊 Found {len(video_formats)} video formats, {len(audio_formats)} audio formats"
                )
                ...
                if not video_formats and not audio_formats:
                    ...
                    for i, (video_id, height, line) in enumerate(video_formats):
                        ...
                        print(
                            f"📹 Trying format {i + 1}/{len(video_formats)}: {format_to_try} ({height}p)"
                        )
                    ...
                    for i, (video_id, height, line) in enumerate(video_formats):
                        print(
                            f"📹 Trying video-only format {i + 1}/{len(video_formats)}: {video_id} ({height}p)"
                        )
